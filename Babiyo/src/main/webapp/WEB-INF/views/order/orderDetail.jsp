...
function cancelFnc(no){
	if(confirm('정말로 주문을 취소하시겠습니까?')){
		...
		$('#orderInfoForm').attr('action','./' + no + '/cancel');
...
function acceptFnc(no){
	if(confirm('정말로 주문접수를 하시겠습니까?')){
		...
		$('#orderInfoForm').attr('action','./' + no + '/accept');
		$('#orderInfoForm').submit();
	}
}
	...
	<jsp:include page="/WEB-INF/views/commonMiddleDiv.jsp" />
		...
		<div id="middleMainDiv">
			...
			<div id="paymentInfoDiv">
				<div id="mealkitListDiv">
					...
					<ul id="mealkitUl">
						<c:forEach items="${orderMap.orderDetailList}" var="mealkit">
							<li>
								<span class="mealkitName">
									<a href="/babiyo/product/${mealkit.productNo}"><strong>${mealkit.productName}</strong></a>
								</span>
				...
				<input id="orderListBtn" class="btn" type="button" value="주문목록" onclick="goListFnc();">
			</div>
